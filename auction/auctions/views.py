...
def bid(request, id):
    if request.method == 'POST':
        ...
        record_data = request.POST["record"]
        print(record_data)
        record = [] 
        record.append(record_data.split(','))
        randomNo = random.randint(1, 10)
        # predicting human or bot
        if predict_human(record):
            if randomNo < 3:
                user = request.user
                bid = Bid.objects.create(
                date=timezone.now(), user=user, bidValue=bidValue, auctionListing=auctionListing, isBot=True)
                bid.save()
                messages.warning(
                    request, f'Bot Detected!')
            else:
                messages.warning(
                    request, f'Bot Detected!')
            #return HttpResponseRedirect(reverse("details", kwargs={'id': id}))
            ...
            user = request.user
            bid = Bid.objects.create(
                date=timezone.now(), user=user, bidValue=bidValue, auctionListing=auctionListing, isBot=False)
            bid.save()
    return HttpResponseRedirect(reverse("details", kwargs={'id': id}))
...
def dashboard(request, itemId):
    ...
    if auctionListing.user == user:
        ...
        messages.info(
            request, 'You are not authorized to see this Dashboard Page!')
    #return HttpResponseRedirect(reverse("dashboard", kwargs={'id': itemId}))
...
def predict_human(record: list):
    ...
    # /Users/sonali/Documents/CMPE-295B_Project_FinalSem_Fall2022/Project_Code_files/JupyterCodeDataset
    model=pickle.load(open('/Users/sonali/Documents/CMPE-295B_Project_FinalSem_Fall2022/Project_Code_files/JupyterCodeDataset/model_rf.pkl', 'rb'))
